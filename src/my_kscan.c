 ...
 */
 ...
 #define USE_POLLING IS_ENABLED(CONFIG_KEYBALL_KSCAN_MATRIX_POLLING)
 #define USE_INTERRUPTS (!USE_POLLING)
 
 #define COND_INTERRUPTS(code) COND_CODE_1(CONFIG_KEYBALL_KSCAN_MATRIX_POLLING, (), code)
 #define COND_POLL_OR_INTERRUPTS(pollcode, intcode)                                                 \
     COND_CODE_1(CONFIG_KEYBALL_KSCAN_MATRIX_POLLING, pollcode, intcode)
 ...
 static int set_pin_as_output(const struct gpio_dt_spec *pin) {
     int err = gpio_pin_configure_dt(pin, GPIO_OUTPUT_LOW);
     if (err) {
         LOG_ERR("Unable to configure pin %u on %s for output", pin->pin, pin->port->name);
         return err;
     }
 
     // LOG_DBG("Configured pin %u on %s for output", pin->pin, pin->port->name);
     return 0;
 }
 ...
 static int kscan_matrix_read(const struct device *dev) {
     ...
     for (int row = 0; row < rows->len; row++) {
 ...
 #if CONFIG_KEYBALL_KSCAN_MATRIX_WAIT_BEFORE_INPUTS > 0
         k_busy_wait(CONFIG_KEYBALL_KSCAN_MATRIX_WAIT_BEFORE_INPUTS);
 #endif
         ...
         for (int col = 0; col < cols->len; col++) {
             ...
             const int active = kscan_gpio_pin_get(col_gpio, &state);
             if (active < 0) {
 ...
 #if CONFIG_KEYBALL_KSCAN_MATRIX_WAIT_BETWEEN_OUTPUTS > 0
         k_busy_wait(CONFIG_KEYBALL_KSCAN_MATRIX_WAIT_BETWEEN_OUTPUTS);
 #endif
     }
     ...
     for (int col = 0; col < cols->len; col++) {
         ...
         for (int row = 0; row < rows->len; row++) {
             ...
             const int active = kscan_gpio_pin_get(row_gpio, &state);
             if (active < 0) {
